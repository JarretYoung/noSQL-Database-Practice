/*
Databases SQL Portfolio 3 2023 S1
...
-- CREATE all PROPINSURANCE tables
...
COMMENT ON COLUMN policy.policy_id IS
    'Inspect item unique identifier (surrogate key)';
...
COMMENT ON COLUMN policy.policy_enddate IS
    'Policy end date';
...
ALTER TABLE policy
    ADD CONSTRAINT policy_nk UNIQUE ( prop_no,
                                      policy_startdate,
                                      policy_type_code );
...
ALTER TABLE property
    ADD CONSTRAINT chk_propfullyfurnished CHECK ( prop_fully_furnished IN ( 'N', 'Y' )
    );
...
ALTER TABLE policy
    ADD CONSTRAINT policytype_policy FOREIGN KEY ( policy_type_code )
